...
const apn = {
  ...
  init: async (username, password, puppeteerOptions = {}) => {
    ...
    apn._viewId = "a3H0h000000pQEbEAM";
  ...
  opportunities: async () => {
    ...
    const viewId = await page.evaluate(() => {
      const ocText = document.querySelectorAll('#editcustomviewlink')[0].onclick.toString();
    ...
    return apn._opportunitiesXLSXtoJSON(opportunityXLSX, { type: "string" });
  },
  certifications: async () => {
    const certifications = await apn._getCSV(
      "https://partnercentral.awspartner.com/PartnerCertificationDetailsExport"
    );
    return apn._certificationsCSVtoJSON(certifications, { type: "string" });
  },
  _getCSV: async (url) => {
